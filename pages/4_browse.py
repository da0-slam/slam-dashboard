            ...
            try:
                ...
                if not _id_col or not _url_col:
                    ...
                    if rows and st.button(f"✅ {len(rows)}행 koc_contents에 업로드", type="primary", key="koc_import_run"):
                        ...
                        # ── 1) CSV에 썸네일 URL 있는 행 → Supabase 병렬 업로드 ──
                        _has_thumb = [r for r in rows if r.get("thumbnail_url") and "supabase" not in (r.get("thumbnail_url") or "")]
                        if _has_thumb:
                            def _upload_one(r):
                                _pid = extract_post_id(r["video_url"]) or r["influencer_id"]
                                _sb = upload_thumbnail(r["thumbnail_url"], r["influencer_id"], _pid)
                                if _sb:
                                    r["thumbnail_url"] = _sb
                                    return True
                                return False
                            with st.spinner(f"썸네일 {len(_has_thumb)}개 Supabase 저장 중..."):
                                with ThreadPoolExecutor(max_workers=6) as _ex:
                                    _res = list(_ex.map(_upload_one, _has_thumb))
                            _thumb_ok = sum(_res)
                            st.caption(f"썸네일: ✅ {_thumb_ok}개 저장  ⚠️ {len(_res)-_thumb_ok}개 실패")

                        # ── 2) 썸네일 없는 행만 → video_url에서 스크랩 ──────────
