...
if is_admin:
    ...
    with st.expander("🔧 [Admin] 게시물 데이터 임포트 (Apify → koc_contents)"):
        st.markdown("""
Google Sheet 또는 CSV 파일에서 Apify 스크래퍼 데이터를 바로 업로드합니다. (TikTok / Instagram 공통)

**TikTok**: `authorMeta/name` → influencer_id · `webVideoUrl` → video_url · `playCount` → play_count ·
`diggCount` → like_count · `commentCount` → comment_count · `shareCount` → share_count · `collectCount` → save_count · `videoMeta/coverUrl` → 썸네일

**Instagram**: `ownerUsername` → influencer_id · `url` → video_url · `videoViewCount` → play_count ·
`likesCount` → like_count · `commentsCount` → comment_count · **`displayUrl` / `images/0` → 썸네일 (Supabase Storage 영구저장)**
""")
        ...
        if _csv_bytes:
            try:
                ...
                _df.columns = [c.strip().lower().replace(".", "_").replace("/", "_") for c in _df.columns]

                _col_set = set(_df.columns)
                ...
                def _fc(candidates):
                    ...
                    for cand in candidates:
                        if cand in _col_set:
                            return cand
                    # 2순위: 포함 매칭 (긴 컬럼명 우선, latestComments/0/text 같은 노이즈 방지)
                    return next(
                        (c for c in _df.columns if any(k in c for k in candidates)
                         and not any(noise in c for noise in ("/", "latest", "comment_text"))),
                        None,
                    )

                # dot-notation (authorMeta.name) 및 underscore (authormeta_name) 모두 인식
                _id_col    = _fc(["authormeta.name", "authormeta_name", "author_name", "influencer_id", "ownerusername", "username"])
                _url_col   = _fc(["webvideourl", "video_url", "videourl", "url"])
                _play_col  = _fc(["videoviewcount", "videoplaycount", "playcount", "play_count"])
                _like_col  = _fc(["likescount", "diggcount", "likecount", "like_count"])
                _cmt_col   = _fc(["commentscount", "commentcount", "comment_count"])
                _shr_col   = _fc(["sharecount", "share_count"])
                _save_col  = _fc(["collectcount", "save_count"])
                _cap_col   = _fc(["text", "caption"])
                _date_col  = _fc(["createtimeiso", "timestamp", "createtime", "posted_at"])
                # videometa_coverurl = 실제 영상 커버; authormeta_avatar = 프로필 사진 (후순위)
                _thumb_col = _fc(["thumbnail_url", "videometa_originalcoverurl", "videometa_coverurl", "videometa_coverimage", "displayurl", "images_0", "cover", "authormeta_avatar", "avatar"])

                st.caption(f"인식된 컬럼: influencer_id={_id_col}, video_url={_url_col}, play={_play_col}, like={_like_col}, caption={_cap_col}")
                ...
                if not _id_col or not _url_col:
                    ...
                    for _, row in _df.iterrows():
                        ...
                        # 반복 헤더행(video_url 컬럼명이 값으로 들어온 경우) 및 비URL 건너뜀
                    ...
                    if rows and st.button(f"✅ {len(rows)}행 koc_contents에 업로드", type="primary", key="koc_import_run"):
                        from utils.storage_client import upload_thumbnail, extract_post_id

                        # ── 썸네일 Supabase Storage 영구저장 ──────────────────
                        from utils.storage_client import fetch_and_upload_thumbnail as _fat
                        _has_thumb = [r for r in rows if r.get("thumbnail_url") and "supabase" not in (r.get("thumbnail_url") or "")]
                        _thumb_ok = _thumb_fail = 0
                        if _has_thumb:
                            _thumb_bar = st.progress(0, text="썸네일 영구저장 중...")
                            for _i, _r in enumerate(_has_thumb):
                                _raw = _r["thumbnail_url"]
                                _iid = _r["influencer_id"]
                                _pid = extract_post_id(_r["video_url"]) or _iid
                                _sb = upload_thumbnail(_raw, _iid, _pid)
                                if _sb:
                                    _r["thumbnail_url"] = _sb
                                    _thumb_ok += 1
                                else:
                                    _thumb_fail += 1
                                _thumb_bar.progress((_i + 1) / len(_has_thumb), text=f"썸네일 {_i+1}/{len(_has_thumb)}")
                                _time.sleep(0.15)
                            _thumb_bar.empty()
                            st.caption(f"썸네일: ✅ {_thumb_ok}개 영구저장  ⚠️ {_thumb_fail}개 실패")

                        # ── displayUrl 없는 행 → video_url에서 직접 스크랩 ──
                        _no_thumb = [r for r in rows if not r.get("thumbnail_url")]
                        if _no_thumb:
                            _sc_bar = st.progress(0, text="썸네일 스크랩 중...")
                            _sc_ok = _sc_fail = 0
                            for _i, _r in enumerate(_no_thumb):
                                _iid = _r["influencer_id"]
                                _pid = extract_post_id(_r["video_url"]) or _iid
                                _sb2 = _fat(_r["video_url"], _iid, _pid)
                                if _sb2:
                                    _r["thumbnail_url"] = _sb2
                                    _sc_ok += 1
                                else:
                                    _sc_fail += 1
                                _sc_bar.progress((_i + 1) / len(_no_thumb), text=f"스크랩 {_i+1}/{len(_no_thumb)}")
                                _time.sleep(0.3)
                            _sc_bar.empty()
                            st.caption(f"스크랩: ✅ {_sc_ok}개  ⚠️ {_sc_fail}개 실패")
