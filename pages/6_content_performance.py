...
from utils.supabase_client import (
    ...
    get_campaign_posts,
    get_campaign_posts_for_campaigns,
    get_campaigns,
...
if is_admin:
    _tab_labels.append("📤 업로드 현황")

_tabs = st.tabs(_tab_labels)
tab1, tab2, tab3, tab4, tab5 = _tabs[:5]
tab6 = _tabs[5] if is_admin else None
...
# ═══════════════════════════════════════════════════════════════
# Tab 6 – 업로드 현황 (어드민 전용, 전체 브랜드 취합)
# ═══════════════════════════════════════════════════════════════

if is_admin and tab6 is not None:
    with tab6:
        st.subheader("📤 전체 프로젝트 업로드 현황")
        st.caption("모든 브랜드의 캠페인을 발송 인원 대비 업로드 인원 기준으로 한눈에 봅니다. "
                    "(현재 화면 상단의 캠페인/브랜드 선택과 무관하게 전체를 표시합니다)")

        _all_camps = get_all_campaigns()
        if not _all_camps:
            st.info("등록된 캠페인이 없습니다.")
        else:
            _brand_map_all = {b["id"]: b["name"] for b in get_brands()}
            ...
            def _upload_row(c: dict) -> dict:
                ...
                return {
                    "브랜드":      _brand_map_all.get(c.get("brand_id"), "-"),
                    "캠페인":      c.get("name", "-"),
